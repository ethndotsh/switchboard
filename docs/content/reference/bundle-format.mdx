...
```json descriptor.json
{
  ...
  "artifacts": {
    ...
    "data/allowlist.txt": { "digest": "sha256:…", "size": 172 }
  },
  "provenance": {
...
| `abi` | yes | Host-function ABI the module targets (`switchboard/v4`). |
| `manifest` | yes | Bundle name, ABI, entrypoint export, source language. |
| `artifacts` | yes | Digest and size of every artifact: `module.wasm`, optional `tests.yaml`, and any [data files](/guides/data-files) under `data/`. |
...
bundles/sha256-<digest>/data/allowlist.txt
bundles/sha256-<digest>/checksum.txt
bundles/sha256-<digest>/descriptor.json
```

`descriptor.json` is written last, so its presence marks a complete upload after every other object (including `data/` files) is durably in place.
...
- Proxies refuse bundles whose `abi` they don't support. The current ABI is `switchboard/v4`; bundles built against `switchboard/v3` or earlier must be rebuilt with the current SDK.
